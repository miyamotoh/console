...
const selectFromDropdown = async (dropdownButton, text) => {
  ...
  await browser.wait(until.presenceOf($('[data-test-id=dropdown-text-filter]'))).then(() => {
    $('[data-test-id=dropdown-text-filter]').sendKeys(text).then(() =>
      element(by.cssContainingText('li[role=option] a', text)).click(),
    );
  });
